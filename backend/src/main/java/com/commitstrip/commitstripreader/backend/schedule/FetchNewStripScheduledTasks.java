...
public class FetchNewStripScheduledTasks {
  ...
  @Transactional
  @Scheduled(fixedDelay = 1800*1000) // Every 30 minutes
  public List<StripDao> fetchNewStrip() {
    ...
    try {
      ...
      if (toSave.size() != 0 && Configuration.shouldStartToFetchNewStrip) {
        // Sort by date
        toSave.sort((o1, o2) -> (o1.getReleaseDate().getTime() > o2.getReleaseDate().getTime() ? -1 : 1));
        ...
        StripDao stripMostRecent = repositoryDatabase.findFirst1ByOrderByReleaseDateDesc();
        ...
        for (int i=0; i<toSave.size(); i++) {
          ...
          if (i == 0)
            toSave.get(i).setNext(0L); // The first next is 0. It a default value.
          else
